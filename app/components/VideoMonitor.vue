<template>
  <div class="relative w-screen h-screen bg-black">
    ...
    <div
      v-if="cameraError"
      class="absolute inset-0 flex flex-col gap-4 items-center justify-center bg-black"
    >
      <h2 class="text-white text-2xl font-bold">Camera Unavailable</h2>
      <p class="text-gray-300 text-center max-w-sm px-4">{{ cameraError }}</p>
      <button
...
const start = async () => {
  if (!stream) {
    throw new Error(
      "Cannot start recording without an initialized camera stream",
    );
  }
...
onMounted(async () => {
  try {
    ...
    if (msg.includes("not available in this browser")) {
      cameraError.value = "Camera access requires a secure connection (HTTPS). Please ensure the app is served over HTTPS.";
